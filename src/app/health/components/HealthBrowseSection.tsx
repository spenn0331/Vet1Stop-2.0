...
/**
 * HealthBrowseSection.tsx — MongoDB-powered browse experience for the Health Hub.
 * Tabs: All | VA | NGO | State — search, tag filters, sort, pagination.
 * Uses /api/health/browse which queries subcategory field directly.
 */
...
type Tab = 'all' | 'federal' | 'ngo' | 'state';

const TABS: { id: Tab; label: string; ariaLabel: string }[] = [
  { id: 'all',     label: 'All',   ariaLabel: 'Browse all health resources' },
  { id: 'federal', label: 'VA',    ariaLabel: 'Browse VA federal resources' },
  { id: 'ngo',     label: 'NGO',   ariaLabel: 'Browse NGO and nonprofit resources' },
  { id: 'state',   label: 'State', ariaLabel: 'Browse state-specific resources' },
];
...
export default function HealthBrowseSection() {
  ...
  const fetchResources = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/health/browse?${params.toString()}`);
      ...
      console.error('[HealthBrowseSection]', err);
    } finally {
      setLoading(false);
    }
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-6">
          <div>
            <h2 id="browse-heading" className="text-2xl font-extrabold text-[#1A2C5B] tracking-tight">
              Browse Health Resources
            </h2>
            <p className="text-sm text-gray-500 mt-0.5">Explore VA, nonprofit, and state programs directly</p>
          </div>
          {/* Sort control */}
              ...
              <div className="absolute right-0 mt-1 w-44 bg-white border border-gray-100 rounded-xl shadow-lg z-10 overflow-hidden">
                {SORT_OPTIONS.map(opt => (
                  <button
                    ...
                    className={`w-full text-left px-4 py-2.5 text-sm transition-colors duration-150 ${sortBy === opt.value ? 'bg-blue-50 text-[#1A2C5B] font-semibold' : 'text-gray-600 hover:bg-gray-50'}`}
        ...
        <div className="relative mb-4">
          ...
          <input
            ...
            placeholder="Search resources, conditions, programs…"
